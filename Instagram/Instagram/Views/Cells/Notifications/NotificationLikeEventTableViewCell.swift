...
//  Instagram
//
//  Created by Evren Ustun on 13.08.2022.
//
...
class NotificationLikeEventTableViewCell: UITableViewCell {
    ...
    public func configure(with model: UserNotification) {
        ...
        switch model.type {
        case .like(let post):
            let thumbail = post.thumbnailImage
            
            guard !thumbail.absoluteString.contains("google.com") else {
                return
            }
            
            postButton.sd_setBackgroundImage(with: thumbail, for: .normal, completed: nil)
        case .follow:
            break
        }
